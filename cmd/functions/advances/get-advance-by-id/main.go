...
func handler(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	advance, err := advanceService.GetAdvanceByID(ctx, advanceID)
	if err != nil {
		if apperr.IsNotFoundError(err) {
			...
			return utils.BuildErrorResponse(http.StatusBadRequest, "invalid advance ID"), nil
		} else {
			return utils.HandleDefaultErrors(err), nil
		}
	}
	responseAdvance := dtos.ToAdvanceDTO(advance)
	return utils.BuildResponse(http.StatusOK, "advance fetched successfully", responseAdvance), nil
}
