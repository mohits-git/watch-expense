...
func handler(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	expense, err := expenseService.GetExpenseByID(ctx, expenseID)
	if err != nil {
		if apperr.IsNotFoundError(err) {
			...
			return utils.BuildErrorResponse(http.StatusBadRequest, "invalid expense ID"), nil
		} else {
			return utils.HandleDefaultErrors(err), nil
		}
	}
	responseExpense := dtos.ToExpenseDTO(expense)
	return utils.BuildResponse(http.StatusOK, "expense fetched successfully", responseExpense), nil
}
