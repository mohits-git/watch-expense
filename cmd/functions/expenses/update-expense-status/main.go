...
func handler(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	updateExpenseStatusRequest, err := utils.DecodeJson[dtos.UpdateExpenseStatusRequest](event.Body)
	if err != nil {
		return utils.BuildErrorResponse(http.StatusBadRequest, "invalid request"), nil
	}
	expenseID := event.PathParameters["id"]
	err = expenseService.UpdateExpenseStatus(ctx, expenseID, updateExpenseStatusRequest.Status)
	if err != nil {
	...
	return utils.BuildResponse(http.StatusOK, "expense status updated successfully", struct{}{}), nil
}
